...
from config import LLM_API_KEY, LLM_BASE_URL, LLM_CHAT_MODEL
from engine.document_store import store

# ── Document summaries ────────────────────────────────────────────────

# How many chars per doc summary. With 100 docs:
#   250 chars × 100 docs ≈ 25K chars ≈ well within 32K-token LLM context
#   Adjust downward if you have more docs or see token-limit errors.
SUMMARY_CHARS_PER_DOC = 250

def _build_doc_summaries() -> list[dict]:
    """
    Build title + compact summary for every indexed document.
    Uses get_compact_summary() which prioritises headings + key paragraphs.
    Scales to 100+ documents without exceeding LLM context limits.
    """
    summaries = []
    for doc_id, doc in store.documents.items():
        summary = store.get_compact_summary(doc_id, max_chars=SUMMARY_CHARS_PER_DOC)
        summaries.append({
            "id": doc_id,
            "title": doc["title"],
            "summary": summary or doc["text"][:SUMMARY_CHARS_PER_DOC],
        })
    return summaries
...
RANKING_SYSTEM = """你是一个专业的 On-Call 搜索引擎排序专家。用户会用自然语言描述他们遇到的运维问题，你需要从所有 SOP 文档中找出最相关的。

## 规则
1. 理解用户查询的真实意图（用户可能使用口语化表达，如"挂了"="服务不可用/宕机/崩溃"）
2. 根据文档的标题和摘要判断哪些文档能最好地回答用户的问题
3. 按相关度从高到低排序，score 范围 0.0～1.0，只返回 score > 0.3 的文档
4. 最多返回 10 个结果
...
def _build_ranking_prompt(query: str) -> str:
    ...
    Scales to 100+ documents: each doc gets ~250 chars in the summary,
    keeping total prompt well within typical 32K-token LLM context windows.
    """
    summaries = _build_doc_summaries()

    docs_text = ""
    for i, s in enumerate(summaries, 1):
        docs_text += f"[{i}] {s['id']} | {s['title']}\n    {s['summary']}\n\n"

    return f"""以下是所有可用的 SOP 文档（共 {len(summaries)} 份）：
...
{{"results": [
  {{"id": "sop-xxx", "score": 0.95, "reason": "简要中文解释"}},
  ...
]}}

注意：score 范围 0.0～1.0，只包含 score > 0.3 的文档。"挂了" = 服务宕机/不可用/崩溃；"被攻击" = 安全事件/入侵/DDoS。理解口语化查询的真实意图。"""
